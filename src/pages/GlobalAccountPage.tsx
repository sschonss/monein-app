...
export default function GlobalAccountPage() {
  ...
  const [tab, setTab] = useState<'all' | 'deposit' | 'return'>('all');
  ...
  const filteredTx = tab === 'all' ? transactions : transactions.filter(t => t.direction === tab);
    ...
    <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      {loading ? (
        ...
        <>
          ...
          <h2 style={{ fontSize: '1rem', fontWeight: 700, marginTop: '0.5rem' }}>Transações</h2>
          <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <Card key={`${tx.id}-${tx.direction}`} style={{ padding: '0.75rem 1rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{
                      ...
                      background: tx.direction === 'return' ? 'rgba(34,197,94,0.1)' : 'rgba(71,85,105,0.1)',
                    }}>
                      {tx.direction === 'return' ? (
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: '0.8125rem', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {tx.direction === 'return' ? 'Resgate AstroPay' : (tx.description || 'Conta Global')}
                      </p>
                      <p style={{ fontSize: '0.625rem', color: 'var(--color-text-muted)' }}>
                        {fmtDate(tx.date)} · {tx.direction === 'return' ? 'Resgate' : tx.currency}
                      </p>
                    </div>
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <p style={{ fontSize: '0.875rem', fontWeight: 600, color: tx.direction === 'return' ? 'var(--color-income)' : '#475569' }}>
                        {tx.direction === 'return' ? '+' : '-'}{mask(fmt(tx.amount_brl))}
                      </p>
                    </div>
                  </div>
                </Card>
